...
function VendorNavbar() {
  ...
  // Load vendor data from localStorage on mount and listen for changes
  useEffect(() => {
    const loadVendor = () => {
      const storedVendor = JSON.parse(localStorage.getItem("vendor"));
      console.log("Loaded vendor:", storedVendor);
    ...
    window.addEventListener("storage", loadVendor);

    return () => {
      window.removeEventListener("storage", loadVendor);
    };
  }, []);
  ...
  const handleLogout = () => {
    ...
    setVendor(null); // ✅ Immediately update UI
    ...
    >
      <div className="max-w-7xl mx-auto flex justify-between items-center p-4">
        ...
        <div className="hidden md:flex items-center space-x-8 text-gray-200 text-lg">
          <Link to="/ManageProducts" className="hover:text-green-400 transition">Manage Products</Link>
          <Link to="/vendor-sales" className="hover:text-green-400 transition">View Sales</Link>
        </div>

        {/* Right Section: Vendor Info + Logout */}
